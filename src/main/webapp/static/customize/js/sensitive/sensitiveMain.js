...
var Page = function() {
    return {
        ...
        listPage : function(curPage,flag) {
            ...
            var remote_url = window.basePath + "sensitive/totalpage";
            $.post(remote_url, params, function(data) {
                ...
                remote_url = window.basePath + "sensitive/list";
                $.post(remote_url, params, function(data) {
        ...
        doManage : function(id,action) {
            var remote_url;
            if(action == "verify"){
                remote_url = window.basePath + "sensitive/verify/" + id;
                window.location.href = remote_url;
            }else if(action == "detail"){
                remote_url = window.basePath + "sensitive/detail/" +id;
                window.location.href = remote_url;
            }else{
