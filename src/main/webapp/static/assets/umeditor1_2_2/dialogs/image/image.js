(function() {
	...
	var utils = UM.utils, browser = UM.browser, Base = {
		...
		scale : function(img, max, oWidth, oHeight) {
			var width = 0, height = 0, percent, ow = img.width || oWidth, oh = img.height
					|| oHeight;
			if (ow > max || oh > max) {
				if (ow >= oh) {
					if (width = ow - max) {
			...
			$img.css({
				...
				left : ($img.parent().width() - $img.width()) / 2
					...
					function() {
						...
						if ($(this).parent().remove().hasClass(
		...
		callback : function(editor, $w, url, state) {
			...
			if (state == "SUCCESS") {
				// 显示图片计数+1
				Upload.showCount++;
				var $img = $("<img src='" + editor.options.imagePath + url
						+ "' class='edui-image-pic' />"), $item = $(
	...
	var Upload = {
		...
		config : function(sel) {
			var me = this, url = me.editor.options.imageUrl;

			url = url + (url.indexOf("?") == -1 ? "?" : "&") + "editorid="
		...
		uploadComplete : function(r) {
			...
			try {
				...
				Base.callback(me.editor, me.dialog, '',
						(lang && lang.uploadError) || 'Error!');
			}
		},
		submit : function(callback) {

			var me = this, input = $('<input style="filter: alpha(opacity=0);" class="edui-image-file" type="file" hidefocus="" name="upfile" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp">'), input = input[0];
		...
		drag : function() {
			...
			if (!UM.browser.ie9below) {
				me.dialog.find('.edui-image-content').on(
						'drop',
						function(e) {
	...
	var NetWork = {
		...
		initEvt : function() {
			var me = this, url, $ele = $(".edui-image-searchTxt", me.dialog);
	...
	var $tab = null, currentDialog = null;
			...
			.registerWidget(
					'image',
					{
						...
						initContent : function(editor, $dialog) {
							var lang = editor.getLang('image')["static"], opt = $
									.extend(
											{},
											lang,
											{
												image_url : UMEDITOR_CONFIG.UMEDITOR_HOME_URL
														+ 'dialogs/image/'
											});
