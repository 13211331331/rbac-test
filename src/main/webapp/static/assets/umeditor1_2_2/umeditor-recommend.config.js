 ...
 */
/*******************************************************************************
 * 提示************************ 所有被注释的配置项均为UEditor默认值。 修改默认配置请首先确保已经完全明确该参数的真实用途。
...
(function() {
			...
			|| (function() {
				...
				function update() {
					...
					if (protocol) {
						...
						// local
						this.localSeparator = /\\|\//.exec(this.path.replace(
								protocol, ''))[0];
	...
	window.UMEDITOR_CONFIG = {
		...
		// 图片上传配置区
		,
		imageUrl : "../../../uploadimg/image?module=recommend" // 图片上传提交地址
		,
		...
		// video source
		,
		toolbar : [ 'undo redo removeformat preview image fullscreen' ]
		...
		,
		initialFrameWidth : 750 // 初始化编辑器宽度,默认500
		,
		...
		// 注意这里添加的样式，最好放在.edui-editor-body
		// .edui-body-container这两个的下边，防止跟页面上css冲突
		// ,initialStyle:'.edui-editor-body .edui-body-container
		// p{line-height:1em}'
